 ...
 * tools) to prevent circular calls: ClawQuant's own AI provider never sees
...
export class McpAskPlugin implements Plugin {
  ...
  async start(ctx: EngineContext) {
    ...
    const createMcpServer = () => {
      const mcp = new McpServer({ name: 'claw-quant-ask', version: '1.0.0' })

      // ── askWithSession ──
      mcp.tool(
        'askWithSession',
        'Send a message to Claw and get a response within a persistent session.',
        { message: z.string().describe('The message to send to Claw'), sessionId: z.string().describe('Session identifier (caller-managed)') },
